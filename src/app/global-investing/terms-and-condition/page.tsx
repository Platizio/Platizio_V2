...
const DOC_ID = "global-terms-and-condition";
 ...
 * of one. The risk is real here rather than theoretical: this route's id
 * ("global-terms-and-condition") is deliberately not its path
 * ("/global-investing/terms-and-condition"), so the two are easy to confuse.
 *
...
if (!entry) {
  throw new Error(
    `No legal document is registered under the id "${DOC_ID}" ` +
      ...
      `src/app/global-investing/terms-and-condition/page.tsx cannot render without it.`,
  );
}
...
export const metadata: Metadata = {
   ...
   * That matters most on this pair of routes: the global documents are near
   * duplicates of the Indian ones, and a hand-typed canonical that dropped the
   * `/global-investing` prefix would quietly point this page at the other
   * terms — the one case where a wrong canonical would get a real page
   * de-indexed in favour of a near-identical twin.
   */
