...
const DOC_ID = "global-privacy-policy";
 ...
 * ("global-privacy-policy") is deliberately not its path
 * ("/global-investing/privacy-policy"), so the two are easy to confuse.
 *
...
if (!entry) {
  throw new Error(
    `No legal document is registered under the id "${DOC_ID}" ` +
      ...
      `src/app/global-investing/privacy-policy/page.tsx cannot render without it.`,
  );
}
...
export const metadata: Metadata = {
   ...
   * privacy policy — the one case where a wrong canonical would get a real
   * page de-indexed in favour of a near-identical twin.
   */
